...
if [ "$1" == "dev" ]; then
  ASPNETCORE_ENVIRONMENT=Development
    ...
    docker run --rm -p 8080:8082 -e ASPNETCORE_ENVIRONMENT=Development totosan/identitykeeper-client:latest &
else
    ...
    docker run --rm -p 8080:8082 -e AZURE_STORAGE_CONNECTION_STRING=$AZURE_STORAGE_CONNECTION_STRING totosan/identitykeeper-client:latest &
fi
